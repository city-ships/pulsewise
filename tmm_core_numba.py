...
import numba as nb
import numpy as np
from numpy import (abs, array, conj, cos, exp, inf, isnan, nan, pi, seterr,
                   sin, zeros)
from numpy.lib.scimath import arcsin

"""
This version of tmm_core is modified to be compatible with 
the Numba JIT compiler for normal incidence calculations to speed it up.
The following changes have been made:

Some functions are decorated with @nb.njit(cache=True) to enable 
Numba jit compilation. Some of them have been rewritten to be compatible
with numba regarding datatypes.

coh_tmm returns not a dict anymore, just the values

Most important for performance  are the find_in_structure_with_inf and 
find_in_structure for the position resolved absorption calulations.

A simple main function has been added for testing an plotting.
...
@nb.njit(cache=True)
def make_2x2_array(a, b, c, d):
    """
    Makes a 2x2 numpy array of [[a,b],[c,d]]
    """
    # Manually check if any inputs are complex to determine dtype
    if (
        isinstance(a, complex)
        or isinstance(b, complex)
        or isinstance(c, complex)
        or isinstance(d, complex)
    ):
        dtype = np.complex128
    else:
        dtype = np.float64
    ...
    my_array[1, 0] = c
    my_array[1, 1] = d
    my_array = my_array.astype(np.complex128)  # cast to complex
    return my_array
...
def is_forward_angle(n, theta):
    """
    Numba-compatible version of is_forward_angle function.
    """
    assert (
        n.real * n.imag >= 0
    ), "For materials with gain, it's ambiguous which beam is incoming vs outgoing."
...
def list_snell(n_list, th_0):
    ...
    using Snell's law. Updated for compatibility with Numba.
    """
...
def interface_r(polarization, n_i, n_f, th_i, th_f):
    ...
    if polarization == "s":
        return np.complex128(
            ((n_i * cos(th_i) - n_f * cos(th_f)) / (n_i * cos(th_i) + n_f * cos(th_f)))
        )
    elif polarization == "p":
        return np.complex128(
            ((n_f * cos(th_i) - n_i * cos(th_f)) / (n_f * cos(th_i) + n_i * cos(th_f)))
        )
    else:
...
def interface_t(polarization, n_i, n_f, th_i, th_f):
    ...
    """
    if polarization == "s":
        return np.complex128(2 * n_i * cos(th_i) / (n_i * cos(th_i) + n_f * cos(th_f)))
    elif polarization == "p":
        return np.complex128(2 * n_i * cos(th_i) / (n_f * cos(th_i) + n_i * cos(th_f)))
    else:
        raise ValueError("Polarization must be 's' or 'p'")
...
def coh_tmm(pol, n_list, d_list, th_0, lam_vac):
    ...
    Outputs the following (see manual for details)
    ...
    * pol, n_list, d_list, th_0, lam_vac--same as input
    ...
    t_list = zeros((num_layers, num_layers), dtype=np.complex128)
    r_list = zeros((num_layers, num_layers), dtype=np.complex128)
    ...
    M_list = zeros((num_layers, 2, 2), dtype=np.complex128)
    ...
    Mtilde = np.dot(
        make_2x2_array(1, r_list[0, 1], r_list[0, 1], 1) / t_list[0, 1], Mtilde
    )
    ...
    vw_list = zeros((num_layers, 2), dtype=np.complex128)
    vw = array([[t], [0.0 + 0j]])
...
def position_resolved(
    ...
    * Ex and Ey and Ez - the electric field amplitudes, where
    ...
    assert (layer >= 1 and 0 <= distance <= d_list[layer]) or (
...
@nb.njit(nb.types.List(nb.float64)(nb.float64[:], nb.float64), cache=True)
...
if __name__ == "__main__":
    ...
    # Parameters
    pol = "s"
    n_list = np.array([1.0, 2 + 0.1j, 1.0])
    d_list = np.array([np.inf, 300, np.inf])
    th_0 = 0.0  # Normal incidence

    # Wavelength sweep
    wavelengths = np.linspace(380, 1200, 1000)  # Visible range
    reflectance = []

    for lam_vac in wavelengths:
        ret = coh_tmm(pol, n_list, d_list, th_0, lam_vac)
        reflectance.append(ret[2])
